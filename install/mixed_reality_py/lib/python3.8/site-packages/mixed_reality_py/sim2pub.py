...
class MultiAgentController(Node):
    ...
    def __init__(self):
        ...
        self.pose_theta_virtual_car = 0.0
        ...
        # create env
        env = F110Env(map=path + conf.map_path, map_ext= conf.map_ext, num_agents=2, seed=args.seed)
        ...
        # 2 cars - 2 agents
        agent_1 = SACAgent(algorithm)
        agent_2 = SACAgent(algorithm)

        # load best model
        best_model_path = path + args.save_root_path + str(args.seed) + '/model/sac_last.ckpt'
        # 2 separated controllers to control 2 agents
        agent_1.restore(best_model_path)
        agent_2.restore(best_model_path)

        # lidar scan for agents
        scans_stack_1 = deque(maxlen=FRAMES)
        scans_stack_2 = deque(maxlen=FRAMES)
        
        # reset env
        initial_list = initialize_episode2()
        obs, step_rewards, done, info, position_goals = env.reset(initial_list)
        ...
        scans_stack_1.append(obs['scans'][0])
        scans_stack_1.append(obs['scans'][0])
        scans_stack_1.append(obs['scans'][0])
        scans_stack_2.append(obs['scans'][1])
        scans_stack_2.append(obs['scans'][1])
        scans_stack_2.append(obs['scans'][1])

        # states for car 1
        scans_1 = copy.deepcopy(scans_stack_1)
        state_1 = np.array([
        ...
        # states for car 2
        scans_2 = copy.deepcopy(scans_stack_2)
        state_2 = np.array([
        ...
        while not done and step < args.max_episode_steps:
            ...
            # action for agent1
            action_1 = agent_1.predict(scans_1, FRAMES, state_1)
            action_1 = maneuver(action_1, info['reach_cond_1'], info['collision_1'])

            # action for agent2
            action_2 = agent_2.predict(scans_2, FRAMES, state_2)
            action_2 = maneuver(action_2, info['reach_cond_2'], info['collision_2'])

            # s' for 2 agents    
            obs_prime, step_rewards, done, info = \
                env.step(np.array([action_1, action_2]), [state_1, state_2], reset_flag=False)
            ...
            # s' for agent1
            scans_stack_1.append(obs_prime['scans'][0])
            state_prime_1 = np.array([target_1_x - obs_prime['poses_x'][0],  target_1_y - obs_prime['poses_y'][0],
                                        obs_prime['poses_theta'][0],
                                        obs_prime['linear_vels_x'][0],
                                        obs_prime['ang_vels_z'][0]])
            # s' for agent2
            scans_stack_2.append(obs_prime['scans'][1])
            state_prime_2 = np.array([target_2_x - obs_prime['poses_x'][1],  target_2_y - obs_prime['poses_y'][1],
                                        obs_prime['poses_theta'][1],
                                        obs_prime['linear_vels_x'][1],
                                        obs_prime['ang_vels_z'][1]])

            # s = s' for 2 agents
            scans_1 = copy.deepcopy(scans_stack_1)
            state_1 = state_prime_1
            scans_2 = copy.deepcopy(scans_stack_2)
            ...
            if not done:
                ...
                episode_reward_1 += step_rewards[0][0]
                episode_reward_2 += step_rewards[1][0]
    ...
    def pose_callback(self, msg):
        ...
        self.pose_x_virtual_car2 = msg.pose.position.x
        print("helloe!!!")
